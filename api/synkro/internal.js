...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (resource === "sync-attempts" || !resource) {
      const attempts = await fetchSyncAttempts(request);
      response.status(200).json({ ok: true, attempts });
      return;
    }

    response.status(400).json({ ok: false, message: "Recurso interno no valido." });
  } catch (error) {
    response.status(500).json({ ok: false, message: error.message });
  }
}

async function fetchSyncAttempts(request) {
...
async function fetchAuditLogs(request) {
